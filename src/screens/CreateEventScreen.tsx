...
export default function CreateEventScreen() {
    ...
    const [endDate, setEndDate] = useState(new Date(new Date().getTime() + 2 * 60 * 60 * 1000)); // Default +2 hours
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Wait for confetti to play before showing alert and going back
            setTimeout(() => {
                setIsSaving(false);
                setShowConfetti(false);
                Alert.alert('Başarılı', 'Etkinlik başarıyla oluşturuldu ve üyelere bildirim gönderildi.', [
                    { text: 'Tamam', onPress: () => navigation.goBack() },
                ]);
            }, 3500);

        } catch (error: any) {
            console.log('Etkinlik oluşturma hatası:', error.response?.data || error);
            Alert.alert('Hata', error.response?.data?.message || 'Etkinlik oluşturulurken bir hata oluştu.');
            setIsSaving(false);
        }
    };
            ...
            >
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={styles.formSection}>
                        {communityName && (
                            <Text style={styles.communitySubText}>Topluluk: {communityName}</Text>
                        )}
                    ...
                    >
                        {isSaving ? (
                            ...
                            <Text style={styles.saveButtonText}>Etkinliği Oluştur</Text>
                        )}
                    </TouchableOpacity>
                </ScrollView>
            </KeyboardAvoidingView>
